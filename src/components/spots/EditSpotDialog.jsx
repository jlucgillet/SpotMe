...
import { Upload, X, Loader2, MapPin, Map, Users, GripVertical, Camera } from "lucide-react";
...
export default function EditSpotDialog({ spot, open, onOpenChange, onSpotUpdated }) {
  ...
  useEffect(() => {
    if (spot) {
      ...
      if (spot.community_ids && Array.isArray(spot.community_ids)) {
        communityIds = spot.community_ids;
      } else if (spot.community_id) {
        communityIds = [spot.community_id];
      }
      
      let categories = ["lieu_remarquable"];
      ...
      setFormData({
        ...
        photos: spot.photos || [],
        trek_details: spot.trek_details || {},
        gpx_file: spot.gpx_file || null
      });
      if (spot.latitude && spot.longitude) {
        setMapCenter([spot.latitude, spot.longitude]);
        setMarkerPosition([spot.latitude, spot.longitude]);
      }
    }
  }, [spot]);
  ...
  const updateMutation = useMutation({
    mutationFn: async (data) => {
      ...
      await base44.entities.Spot.update(spot.id, spotData);
      
      await base44.entities.Notification.create({
        type: "spot",
        message: `${user?.full_name || user?.email || 'Un utilisateur'} a modifié le spot "${data.name}"`,
        related_id: spot.id,
      ...
      if (notifyCommunity) {
        for (const communityId of data.community_ids) {
          ...
          if (community) {
            await base44.entities.Notification.create({
              type: "spot",
              message: `Le spot "${data.name}" a été modifié dans la communauté "${community.name}"`,
              related_id: spot.id,
              related_name: data.name
            });
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9fafb;">
  <div style="background: linear-gradient(135deg, #10b981 0%, #0d9488 100%); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
    <h1 style="color: white; margin: 0; font-size: 28px;">🔄 Spot mis à jour sur <a href="https://spotme.base44.app/" style="color: #d1fae5; text-decoration: none;">SpotMe</a></h1>
  </div>
  ...
  <div style="background: white; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    ...
    <p style="font-size: 16px; margin-bottom: 20px;">
      <strong>${user?.full_name || user?.email || 'Un membre'}</strong> a mis à jour le spot dans la communauté <strong style="color: #10b981;">"${community.name}"</strong> !
    </p>
  ...
  <div style="text-align: center; padding: 20px; color: #9ca3af; font-size: 12px;">
            ...
            await Promise.all(
              uniqueRecipients.map(email =>
                base44.integrations.Core.SendEmail({
                  ...
                  to: email,
                  subject: `🔄 Spot mis à jour : ${data.name}`,
                  body: emailBody
    ...
    onSuccess: () => {
      ...
      toast.success("Spot modifié avec succès !");
      onSpotUpdated();
    },
    onError: () => {
      toast.error("Erreur lors de la modification du spot");
    }
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto z-[10000]">
        <DialogHeader>
          <DialogTitle className="text-2xl">Modifier le spot</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className={`space-y-2 border rounded-md p-3 max-h-48 overflow-y-auto ${
                    ...
                    {userCommunities.length === 0 ? (
                      <p className="text-sm text-red-600 font-medium">
                        Vous n'avez aucune communauté disponible.
                      </p>
                    ) : (
              ...
              <TabsContent value="address" className="space-y-4 mt-4">
                ...
                {formData.latitude && formData.longitude && (
                  <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-200">
                    <p className="text-sm font-medium text-emerald-900">Position actuelle</p>
          ...
          <div className="flex items-center space-x-2 p-4 bg-emerald-50 rounded-lg border border-emerald-200">
            <Checkbox
              id="notify-community-edit"
              checked={notifyCommunity}
              onCheckedChange={setNotifyCommunity}
            />
            <label
              htmlFor="notify-community-edit"
              className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer"
            >
              Avertir ma communauté par email
            </label>
          </div>
